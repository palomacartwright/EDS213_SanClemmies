...
zoobenthos <- read_csv("data/NonZebra_Mendota/zoobenthos_EDI.csv") %>% 
  ...
  select(year, month, depth, transect, replicate, number, biomass, species_level, phylum_order, area)
...
zebra_mendota <- read_csv("data/Zebra_Mendota/zebramusseldensity_EDI.csv") %>% 
  ...
  select(transect, depth, replicate, totalsample_count, area, density_m2, date, year)
```
